...
import { CATS, HANS, STORES, AREAS } from "./han-data.js";
...
const pickW = (r, list) => {
  // list: [[deger, agirlik], ...]
  const tot = list.reduce((n, x) => n + x[1], 0);
  let v = r() * tot;
...
const P = (id, name, kind, semt, floors, units, mix, lat, lng, sector) =>
...
export const BULK_APPROVED = ["yildiz", "buyukvalide", "kurkcu", "laleli-tekstil", "imc", "sultan", "hidayet", "zincirli", "misir-carsisi", "arasta"];
...
export function groupLabel(word, lang) {
  if (!word) return "";
  if (!lang || lang === "tr") return word;
  const e = GW_L10N[word];
...
const LANG_SETS = [["tr"], ["tr"], ["tr"], ["tr", "en"], ["tr", "en"], ["tr", "en", "ru"], ["tr", "en", "ar"], ["tr", "ar"], ["tr", "en", "ru", "ar"]];
...
const STATUS_MIX_BULK = [["beyan", 8], ["onayli", 58], ["aktif", 34]];
const STATUS_MIX_FIELD = [["beyan", 38], ["onayli", 47], ["aktif", 15]];

function corridorOf(r, place, floor) {
  if (place.kind === "cadde") return null;
  const letters = ["A", "B", "C", "D", "E"];
  return letters[int(r, 0, Math.min(4, Math.max(1, Math.round(place.units / 120))))];
}

function makeName(r, cat, sector) {
...
function bandFor(r, cat) {
  const base = { kilif: 25, sarj: 60, tekstil: 45, poset: 8, taki: 900, bijuteri: 20, hali: 3200, deri: 320,
    baharat: 90, hediyelik: 60, gida: 70, kitap: 40, hizmet: 250, imalat: 150, yemek: 180, kahve: 45, tatli: 220 }[cat] || 50;
...
function build() {
  ...
  PLACES.forEach(place => {
    ...
    for (let i = 0; i < share; i++) {
      ...
      records.push({
        ...
        sector, status, approvedVia, bulk,
        // Faz 0.1/0.3/0.4 · ticaret yeteneği ve mod başına koşullar
        trade: {
          sells, quoteBased,
          perakende: sells.includes("perakende") ? { band: retailBand, moq: 1 } : null,
  ...
  const placeOfHan = { yildiz: "yildiz", buyukvalide: "buyukvalide", zincirli: "zincirli", kurkcu: "kurkcu", sultan: "sultan", sark: "sark" };
  const areaPlace = { kapalicarsi: "kapalicarsi-carsi", misircarsisi: "misir-carsisi", nuruosmaniye: "nuruosmaniye-cd", tahtakale: "tahtakale-cd", mahmutpasa: "mahmutpasa-yokusu" };
  ...
  STORES.forEach(s => {
    ...
    records.unshift({
      ...
      status: "aktif", approvedVia: "saha", bulk: true, officer: OFFICER_OF_SEMT[pl ? pl.semt : "kapalicarsi"] || "of-kemal", langs: (s.commerce || {}).languages || ["tr"],
      moq: ((s.trade || {}).minOrder || {}).qty || 1, moqFlex: !!(s.trade || {}).moqFlexible,
      trade: (() => {
        const t = s.trade || {}, type = t.type || "perakende";
        ...
        const sells = type === "ikisi" ? ["toptan", "perakende"]
          : [type].concat(type === "toptan" && hasRetail ? ["perakende"] : [])
                  .concat(type === "perakende" && hasWhole && (t.minOrder || {}).qty > 1 ? ["toptan"] : []);
        const wq = ((t.minOrder || {}).qty) || 12;
        const wB = curatedTrade(s, "toptan"), rB = curatedTrade(s, "perakende");
        return {
          sells, quoteBased: false,
          perakende: sells.includes("perakende") ? { band: rB.band, moq: 1 } : null,
          toptan: sells.includes("toptan") ? { band: wB.band, moq: wq, carton: { inner: wq, unit: "adet" } } : null,
          scope: null
        };
      })(),
      ...
      band: curatedTrade(s, (s.trade || {}).type === "perakende" ? "perakende" : "toptan").band,
...
function applyFreshness(records) {
...
export function applyReports(reportsByRecord) {
  const out = [];
...
export function loadDrafts(onAdd) {
  const out = [];
  try {
...
export function applyApprovals(log) {
  const out = [];
  Object.keys(log || {}).forEach(id => {
    ...
    rec.approvedVia = dec.via || rec.approvedVia;
...
export const SPONSORS = (() => {
  ...
  Object.keys(byCat).forEach(cat => {
    byCat[cat].slice(0, 2).forEach((rec, i) => {
      out.push({
        recordId: rec.id, kind: i === 0 ? "kategori" : "yer", cat, place: rec.place,
...
export function sponsorsFor(kind, key) {
  if (!SETTINGS.showSponsored.value) return [];
  return SPONSORS.filter(s => s.kind === kind && !s.paused && (kind === "kategori" ? s.cat === key : s.place === key))
    .map(s => Object.assign({ sponsor: s }, RECORDS.find(r => r.id === s.recordId) || {}))
    .filter(x => x.id);
}

export function recordsOfPlace(id) { return RECORDS.filter(x => x.place === id); }
...
export function settingImpact(key) {
  if (key === "showDeclared") {
...
export function loadSponsors() {
  try {
    const ov = JSON.parse(localStorage.getItem(SPONSOR_KEY) || "{}") || {};
    (ov.custom || []).forEach(s => {
...
export function sponsorId(s) { return s.recordId + ":" + s.kind; }
function writeSponsors(mut) {
  let ov = {};
  try { ov = JSON.parse(localStorage.getItem(SPONSOR_KEY) || "{}") || {}; } catch (e) {}
  ov.patch = ov.patch || {}; ov.custom = ov.custom || []; ov.dropped = ov.dropped || [];
  mut(ov);
  try { localStorage.setItem(SPONSOR_KEY, JSON.stringify(ov)); } catch (e) {}
}
export function setSponsor(id, patch) {
  const s = SPONSORS.find(x => sponsorId(x) === id);
  if (s) Object.assign(s, patch);
  writeSponsors(ov => { ov.patch[id] = Object.assign({}, ov.patch[id], patch); });
  return s;
}
export function addSponsor(rec) {
...
export function dropSponsor(id) {
...
export function loadPlaces() {
  try {
    const ov = JSON.parse(localStorage.getItem(PLACE_KEY) || "{}") || {};
...
function writePlaces(mut) {
  let ov = {};
  try { ov = JSON.parse(localStorage.getItem(PLACE_KEY) || "{}") || {}; } catch (e) {}
  ov.patch = ov.patch || {}; ov.custom = ov.custom || []; ov.bulk = ov.bulk || []; ov.unbulk = ov.unbulk || [];
  mut(ov);
  try { localStorage.setItem(PLACE_KEY, JSON.stringify(ov)); } catch (e) {}
}
...
export function setBulkApproved(id, on) {
  const i = BULK_APPROVED.indexOf(id);
...
export function isReadOnly(role) { return !!(ROLES[role] || {}).readOnly; }
...
export function auditLog() {
  ...
  const rd = (k, def) => { try { return JSON.parse(localStorage.getItem(k) || def) || JSON.parse(def); } catch (e) { return JSON.parse(def); } };

  const ap = rd("han-approvals-v1", "{}");
...
export function openState(place, now, rec) {
  // Yer nesnesi yerine id geçilirse sessizce han saatine düşmesin.
  const pl = (place && place.kind) ? place : PLACES.find(p => p.id === (place && place.id ? place.id : place));
  const base = HOURS_BY_KIND[pl && pl.kind] || HOURS_BY_KIND.han;
...
export function accessOf(place) {
  const seed = place.id.split("").reduce((a, c) => a + c.charCodeAt(0), 0);
  const hasLift = place.floors.length <= 2 ? true : (place.kind === "is_merkezi" || seed % 3 === 0);
  return {
    lift: hasLift,
...
export function GROUP_WORDS_FOR(cat, lang) {
...
export function tradeFor(rec, mode) {
  const t = rec.trade || {};
