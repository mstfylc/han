...
import { RECORDS, PLACES, SEMTLER, STATUS, SECTORS, CATS_EXTRA, SETTINGS, sellsIn, tradeFor } from "./han-scale.js";
import { candidatesFor, getIndex, stats as indexStats } from "./han-index.js";
import { CATS, CAT_GROUPS } from "./han-data.js";

const TRMAP = { "ı": "i", "İ": "i", "ş": "s", "ğ": "g", "ü": "u", "ö": "o", "ç": "c", "â": "a", "î": "i", "û": "u" };
export const norm = (s) => String(s || "").toLowerCase().replace(/[ıİşğüöçâîû]/g, c => TRMAP[c] || c)
...
const CAT_OF_WORD = (() => {
  ...
  [].concat(CATS || [], CATS_EXTRA || []).forEach(c => {
    ["tr", "en", "ru", "ar"].forEach(l => { if (c[l]) norm(c[l]).split(" ").forEach(w => { if (w.length > 2 && !m[w]) m[w] = c.id; }); });
  });
  return m;
})();
...
export function addSynonym(catId, word) {
...
export function synonymOwner(word) {
  const n = norm(word);
  return n ? (CAT_OF_WORD[n] || null) : null;
}

export function synonymsOf(catId) { return (SYNONYMS[catId] || []).slice(); }

export function dropSynonym(catId, word) {
  const n = norm(word);
...
export function loadLexicon() {
  try {
    const lx = JSON.parse(localStorage.getItem(LEXICON_KEY) || "{}") || {};
...
export function saveLexicon(catId, word, remove) {
  let lx = {};
  try { lx = JSON.parse(localStorage.getItem(LEXICON_KEY) || "{}") || {}; } catch (e) {}
  ...
  try { localStorage.setItem(LEXICON_KEY, JSON.stringify(lx)); } catch (e) {}
  return lx;
}
...
const RECBYID = (() => { const m = new Map(); RECORDS.forEach(r => m.set(r.id, r)); return m; })();
const PLACEBYID = (() => { const m = new Map(); PLACES.forEach(p => m.set(p.id, p)); return m; })();
...
export function indexRecord(rec) {
...
export function parseQuery(q) {
  ...
  const n = norm(raw);
  const out = { raw, n, cats: [], places: [], semtler: [], door: null, phone: null, words: [], kind: "bos" };
  if (!n) return out;
...
// ── (2) aday havuzu ───────────────────────────────────────────────────────
function candidates(pq) {
  if (!pq.n) return RECORDS.map(r => ({ rec: r, match: 0 }));
  const hit = new Map();
  const bump = (id, w) => hit.set(id, (hit.get(id) || 0) + w);
  pq.words.forEach(w => {
    // tam token
  ...
  if (pq.phone) RECORDS.forEach(r => { if (String(r.tel || "").includes(pq.phone)) bump(r.id, 6); });
  const out = [];
...
// ── (3) sıralama ──────────────────────────────────────────────────────────
const STATUS_SCORE = { aktif: 30, onayli: 16, beyan: -14, askida: -40 };

// Sıralama gerekçesi kullanıcıya gösterilir: keyfî görünmesin.
export function reasonsOf(rec, ctx) {
  const out = [];
...
export function scoreOf(rec, match, ctx) {
  let s = (match || 0) * 6;
...
const PUBLIC = (r) => r.status === "aktif" || r.status === "onayli" ||
  (r.status === "beyan" && SETTINGS.showDeclared.value);
export const canPrice = (r) => r.status === "aktif" ||
...
export function search(q, filters, ctx) {
  const F = filters || {}, C = ctx || {};
  const pq = parseQuery(q);
  let pool = candidates(pq);
  ...
  const strict = !!pq.n;
  const kept = [];
  const facets = { semt: {}, place: {}, status: {}, sector: {}, lang: {}, flag: { shipsAbroad: 0, taxFree: 0, producer: 0 } };
  scan.forEach(x => {
    const r = x.rec;
...
export async function searchAsync(q, filters, ctx) {
  await getIndex();
...
export function placeOf(id) { return PLACEBYID.get(id) || null; }
export function statusOf(key) { return STATUS[key] || STATUS.birim; }
export function sectorOf(key) { return SECTORS[key] || SECTORS.perakende; }
...
export function productsIn(cat, ctx) {
  ...
  recs.forEach(r => {
    (r.groups || []).forEach(g => {
      ...
      if (!key) return;
      const e = by.get(key) || { slug: key, name: g.name, cat, shops: 0, lines: 0, lo: Infinity, hi: 0, recs: [] };
      e.shops += 1;
      e.lines += g.lines || 0;
  ...
  return [...by.values()]
    .map(e => ({ ...e, lo: e.lo === Infinity ? null : e.lo, band: e.lo === Infinity ? null : [e.lo, e.hi] }))
    .sort((a, b) => b.shops - a.shops);
}

// Tek ürün: satan dükkânlar sıralı, fiyat dağılımı ve üretici sayısıyla.
export function productDetail(cat, slug, ctx) {
  const C = ctx || {};
  ...
  const shops = p.recs
    ...
    .map(r => {
      const g = (r.groups || []).find(x => productSlug(x.name) === slug) || {};
  ...
  const prices = shops.map(s => s.lo).filter(v => v > 0).sort((a, b) => a - b);
  ...
  return {
    ...
    minMoq: shops.reduce((m, s) => Math.min(m, s.moq || 1), Infinity)
  };
}

// Arama sonucundan ürüne köprü: sorgu bir çeşit grubuna oturuyorsa onu döndür.
export function productForQuery(q, ctx) {
...
export function distribute(req, ctx) {
  const C = ctx || {};
...
export function offersFor(req, ctx) {
  const C = ctx || {};
  ...
  return d.sent.slice(0, 8).filter(rec => !exclude.includes(rec.id)).filter(() => rnd() < 0.55).map(rec => {
    const t = rec.trade || {};
...
export function mergedOffers(req, real, ctx) {
  const rl = (real || []).map(o => {
