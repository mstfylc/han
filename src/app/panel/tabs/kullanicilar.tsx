...
export default function Kullanicilar({ readOnly, refresh, say }: PanelTabProps) {
  ...
  // yönetim kullanıcısına söylenir. Telefona göre anahtarlanır — kullanıcı
  // listesi rehber deposundan, hesaplar `ops_users`'tan gelir; ikisini
  // birbirine bağlayan şey telefondur (K10).
  const [pins, setPins] = useState<Record<string, boolean>>({});
  useEffect(() => {
    let live = true;
    fetch("/api/users", { cache: "no-store" })
      .then((r) => r.json())
      .then((b) => { if (live) setPins(b.pins || {}); })
      .catch(() => {});
    return () => { live = false; };
  }, []);
  // `normTel` sunucuda rakamlara indirger; anahtar aynı biçimde kurulmalı.
  const telKey = (t: string) => String(t || "").replace(/\D/g, "");

  const R = SC.ROLES;
  const users = AD.allUsers();
  const needle = q.trim().toLocaleLowerCase("tr");
  const rows = users.filter((u) => !needle || (u.name + " " + u.tel).toLocaleLowerCase("tr").includes(needle));
  const withPin = users.filter((u) => pins[telKey(u.tel)]).length;
                ...
                <Button
                  ...
                  onClick={() => {
                    void fetch("/api/auth", {
                      ...
                      body: JSON.stringify({ action: "reset", tel: u.tel, name: u.name }),
                    })
                      .then((res) => res.json())
                      .then((r: { ok?: boolean; devCode?: string; delivered?: boolean }) => {
                        if (!r.ok) return say("Kod üretilemedi — yönetici oturumu gerekir");
                        // Kod yalnız geliştirmede döner; üretimde kanal
                        // taşır, ekranda görünmez.
                        setCodes((s) => ({ ...s, [u.id]: r.devCode || null }));
                        refresh();
                        say(r.delivered ? "Sıfırlama kodu gönderildi" : "Sıfırlama kodu üretildi");
                      })
                      .catch(() => say("Kod üretilemedi"));
                  }}
                >
